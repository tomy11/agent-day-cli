...
export class MistralProvider implements LlmProvider {
  ...
  public constructor(options: MistralProviderOptions) {
    ...
    this.baseUrl = normalizeBaseUrl(options.baseUrl ?? 'https://api.mistral.ai/v1')
    ...
    if (!this.apiKey) {
      throw new AppError('MISTRAL_API_KEY_MISSING', 'MISTRAL_API_KEY is required for provider.type=mistral', {
    ...
    try {
      const response = await fetch(`${this.baseUrl}/chat/completions`, {
        ...
        body: JSON.stringify({
          ...
          ...(this.tools.length > 0 ? {tools: this.tools.map(toMistralTool), tool_choice: 'auto'} : {}),
        }),
        signal: shouldUseTimeout ? controller.signal : undefined,
      })

      const rawBody = (await response.json()) as MistralChatResponse

      if (!response.ok) {
        const detail = rawBody.error?.message ?? `HTTP ${response.status}`
        throw new AppError('MISTRAL_HTTP_ERROR', `Mistral request failed: ${detail}`, {
      ...
      if (!message) {
        throw new AppError('MISTRAL_RESPONSE_ERROR', 'Mistral response missing assistant message', {
      ...
      if (toolCalls.length === 0 && content.length === 0) {
        throw new AppError('MISTRAL_RESPONSE_ERROR', 'Mistral response missing content and tool calls', {
          meta: {model: this.model},
        })
      }
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new AppError(
          'MISTRAL_TIMEOUT',
          `Mistral request timed out after ${this.timeoutMs}ms (model: ${this.model}).`,
      ...
      throw new AppError('MISTRAL_NETWORK_ERROR', `Mistral network error: ${getErrorMessage(error)}`, {
        cause: error,
  ...
  public async embed(request: EmbeddingRequest): Promise<EmbeddingResponse> {
    if (!this.embeddingModel) {
      throw new AppError('MISTRAL_RESPONSE_ERROR', 'Mistral embeddingModel is not configured', {
        meta: {provider: 'mistral'},
      })
    }
    ...
    try {
      ...
      const rawBody = (await response.json()) as MistralEmbeddingResponse

      if (!response.ok) {
        const detail = rawBody.error?.message ?? `HTTP ${response.status}`
        throw new AppError('MISTRAL_HTTP_ERROR', `Mistral embeddings request failed: ${detail}`, {
      ...
      if (embeddings.length !== request.inputs.length) {
        throw new AppError('MISTRAL_RESPONSE_ERROR', 'Mistral embeddings response count did not match inputs', {
      ...
      if (error instanceof Error && error.name === 'AbortError') {
        throw new AppError(
          'MISTRAL_TIMEOUT',
          `Mistral embeddings request timed out after ${this.timeoutMs}ms (model: ${this.embeddingModel}).`,
      ...
      throw new AppError('MISTRAL_NETWORK_ERROR', `Mistral embeddings network error: ${getErrorMessage(error)}`, {
        cause: error,
...
function normalizeIndexedEmbeddings(data: MistralEmbeddingResponse['data']): number[][] {
  return (data ?? [])
    .slice()
